...
%	setup_call_cleanup(open('bananas.txt',append,Stream,[alias(myout1)]),
%       (format(myout1,"XXXXXXBBBBB",[])),
%	close(Stream)).
...
get_intent(DictIn,IntentName):-
	setup_call_cleanup(open('/dev/null',append,Stream,[alias(myoutx)]),
	(format(myoutx,"BBBBB",[]),
